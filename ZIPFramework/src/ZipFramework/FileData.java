...
public class FileData extends ZIPObject {
    ...
    private void EncryptFileData() throws UnsupportedEncodingException, IOException
        {
            ...
            BufferedOutputStream bos = new BufferedOutputStream(newTempStream, 2048);
            ...
            try
            {
                ...
                while (bb.hasRemaining())
                {
                    ...
                    bos.write(EncodeByte(keys, bb.get()));
                    bytecount ++;
                }
            }
            finally
    ...
    private byte DecryptByte(int[] keys)
        {
            short temp = (short)(keys[2] | 2);
            return (byte)((temp * (temp ^ 1)) >> 8);
        }
    
    
    
    private int crc32Method(int CRC, byte b)
        {
        int i = b & 0xFF;     
        
        long l = (long)CRC & 0xFFFFFFFFL;
        
        
        long crcnum, crcin;
        
        int tempcrc = crcNums[(i ^ (int)(l)) & 0xff];
        
        long holdRes = tempcrc ^ (l >> 8);
        
        
        
               
        int ret = (int)(holdRes & 0xFFFFFFFFL);
    ...
    private void updateKeys(int[] keys, byte b)
        {
    ...
    private void storedCompression() throws FileNotFoundException, IOException
        {
            ...
            {
                crc32 = new CRC32();
                input = new FileInputStream(this.parent.getFilePath());
                ...
                int offset = 0;
                int numberRead = 0;
                while ((numberRead = input.read(inBytes, 0, 1024)) != -1)
                {
                    crc32.update(inBytes, 0,numberRead);
                ...
                long c32 = crc32.getValue();
                this.parent.getLfh().setCRC32((int)crc32.getValue());
    ...
    private void deflateCompression() throws FileNotFoundException, IOException
        {
            FileInputStream input = null;
            FileOutputStream output = null;
            ...
            try
            {
                ...
                dos = new DeflaterOutputStream(output,d);
                ...
                while ((numberRead = input.read(inBytes, 0, 1024)) != -1)
                {
                    ...
                    dos.write(inBytes, 0, numberRead);
